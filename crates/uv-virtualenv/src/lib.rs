...
pub enum Error {
    ...
    #[error("{}", if *centralized {
        format!("A {name} already exists in the centralised environment store. Use `--clear` to replace it")
    } else {
    ...
    Exists {
        ...
        path: PathBuf,
        /// Whether the environment is in the centralised environment store.
        centralized: bool,
    },
}
