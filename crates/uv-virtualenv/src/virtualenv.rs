    ...
    match location.metadata() {
        ...
        Ok(metadata) if metadata.is_dir() => {
            ...
            let err = Err(Error::Io(io::Error::new(
                ...
                if centralized {
                    format!(
                        "A {name} already exists in the centralised environment store\n\n{}{} {hint}",
                        "hint".bold().cyan(),
                        ":".bold(),
                    )
                } else {
        ...
        Ok(_) => {
            ...
            return Err(Error::Io(io::Error::new(
                io::ErrorKind::AlreadyExists,
                if centralized {
                    "Object already exists in the centralised environment store".to_string()
                } else {
...
fn confirm_clear(
    ...
    if term.is_term() {
        let prompt = if centralized {
            format!(
                "A {name} already exists in the centralised environment store. Do you want to replace it?"
            )
        } else {
            format!(
